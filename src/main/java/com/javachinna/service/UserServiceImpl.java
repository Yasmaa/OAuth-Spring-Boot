...
public class UserServiceImpl implements UserService {
	...
	public User registerNewUser(final UserRegistrationForm userRegistrationForm) throws UserExistAuthenticationException {
		if (userRegistrationForm.getUserID() != null && userRepository.existsById(userRegistrationForm.getUserID())) {
			throw new UserExistAuthenticationException("User with User id " + userRegistrationForm.getUserID() + " already exist");
		} else if (userRepository.existsByEmail(userRegistrationForm.getEmail())) {
			throw new UserExistAuthenticationException("User with email id " + userRegistrationForm.getEmail() + " already exist");
		}
	...
	public LocalUser processUserRegistration(String registrationId, Map<String, Object> attributes, OidcIdToken idToken, OidcUserInfo userInfo) {
		OAuthUserInfo oAuth2UserInfo = OAuth2UserInfoFactory.getOAuth2UserInfo(registrationId, attributes);
		if (StringUtils.isEmpty(oAuth2UserInfo.getName())) {
			throw new OAuthAuthenticationException("Name not found from OAuth2 provider");
	...
	private UserRegistrationForm toUserRegistrationObject(String registrationId, OAuthUserInfo oAuth2UserInfo) {
		return UserRegistrationForm.getBuilder().addProviderUserID(oAuth2UserInfo.getId()).addDisplayName(oAuth2UserInfo.getName()).addEmail(oAuth2UserInfo.getEmail())
				.addSocialProvider(Utils.toSocialProvider(registrationId)).addPassword("changeit").build();
	}
}
